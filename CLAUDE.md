...
- **Slugs are localized into each language.** Translate the item slug per locale so the whole URL is native — EN `/activities/freediving/` → IT `/it/attivita/apnea/`, ES `/es/actividades/apnea/`, FR `/fr/activites/apnee/`. Brand/proper nouns stay as-is (`ikigai-experience`, `catana-47`, `pacific-crossing`). The localized slug carries the local keyword. (Section roots already localize via `src/i18n/segments.ts`; item slugs use the same registry pattern.)
- **Any URL change ships with a 301 — always.** Renaming, translating or moving a slug means adding the old URL (every locale's previous form) → new in `scripts/gen-redirects.ts` (or the content's `oldUrls`). Never change a URL without the redirect; the `ikigaisailing.com` cutover and existing rankings depend on it. The link-localization guard + route tests must stay green.
- **Internal links in body markdown are written EN-canonical** (`/activities/x/`, `/panama/san-blas/x/`, `/benefits/`). A rehype plugin (`src/lib/rehype-localize-links.ts`, wired in `astro.config.mjs`) auto-localizes them per file locale at build (`/it/` page → `/it/attivita/x/`). **Never hardcode `/it/…` in markdown, and never leave a non-EN page linking to an EN URL** — a test (`rendered-html.spec.ts`) fails the build if you do. Component links use `localizePath()`.
